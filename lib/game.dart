...
class Word with IterableMixin<Letter> {
  ...
  factory Word.fromString(String guess) {
    // Sử dụng .characters để tách chữ cái tiếng Việt chuẩn xác (Ví dụ: "ế" là 1 chữ thay vì e + ^ + ')
    var list = guess.trim().toLowerCase().characters.take(5).toList();
    var letters = list
...
extension WordUtils on Word {
  ...
  Word evaluateGuess(Word other) {
    // Thay vì dùng assert(isLegalGuess) làm app dừng đột ngột, 
    // chúng ta chỉ cần xử lý nếu nó hợp lệ, nếu không trả về chính nó.
    if (!isLegalGuess) return this;
